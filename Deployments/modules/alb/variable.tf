...
variable "alb_listener80" {
  ...
  default = {
    port     = 80
    protocol = "HTTP"  # Ensure consistency with target group protocol
  }
}
...
variable "aws_lb_target_group" {
  ...
  default = {
    ...
    port        = 80
    protocol    = "HTTP"  # Ensure consistency with listener protocol
    target_type = "instance"  # Typically "instance" for EC2 targets
  }
}
...
variable "aws_ilb_target_group" {
  description = "Target group for ILB"
  type = object({
    name        = string
    port        = number
    protocol    = string
